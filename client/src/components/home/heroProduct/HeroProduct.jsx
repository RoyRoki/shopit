...
const HeroProduct = ({ product_id }) => {
      ...
      const [heroProduct, setHeroProduct] = useState(productsMap[product_id] || null);
      ...
      const handleAddToCart = async (isBuy) => {
            ...
            if(auth.profileMode === role.guest) {
                  ...
                  const existingCartItem = cart.cartItems.find(cartItem => String(cartItem?.product?.id) === String(heroProduct.productId));
                  
                  if(existingCartItem) {
                        existingCartItem.quantity = existingCartItem?.quantity + quantity;
                        existingCartItem.isAvailable = heroProduct.stock >= existingCartItem?.quantity;
                  } else {
                        cart.cartItems.push({
                              id: 0,
                              quantity: quantity,
            ...
            // For User
                  ...
                  const response = await request("PUT", urls.updateCart, {[product_id] : quantity} )
  ...
  return (
      <div className={styles.main_wrap}>
            <div className={styles.main_container}>
                  ...
                  <div 
                        className={`${styles.cart_notifier_box} ${cartNotifier ? styles.notifier_show : styles.notifier_hide}`}
                  >
                        <div className={styles.highlight_box}>
                              <div className={styles.highlight_img_wrap}>
                                    <div
                                          className={`${styles.hero_img_frame} ${heroImg === '' ? styles.demo_hero_img : "" }`}
                                          style={{backgroundImage: `url(${heroImg})`}}
                                    >
                                    </div>
                              </div>
                              <div className={styles.highlight_text_box}>
                                    <span className={styles.add_to_cart_span}><FontAwesomeIcon icon={faCircleCheck} /> Added to cart</span>
                                    <span>Quantity
                                          {` ${quantity}`}
                                    </span>
                              </div>
                        ...
                        <div className={styles.action_box}>
                              <div className={styles.action_btn_wrap}>
                                    <button className={styles.buy_btn} onClick={() => navigate("/cart")}>Proceed to Buy</button>
                                    <button className={styles.to_cart_btn} onClick={() => navigate("/cart")} >Go to Cart</button>
                  ...
                  <div className={styles.hero_container}>
                        <div className={styles.product_header}>
                                          ...
                                          { heroProduct?.imageUrls?.map((src, index) => (
                                                <div
                                                      ...
                                                      style={{backgroundImage: `url(${src})`,
                                                            border:  `${src === heroImg ? "2px" : "0px" } solid var(--bs-tx2)`}}
                              ...
                              <div className={styles.select_quantity_option} onClick={() => setDropdown(!dropdown)}>
                                <span>
                        ...
                        <div className={styles.product_meta}>
                              ...
                              <div className={styles.prices_info}>
                                    ...
                                    <p>{`₹${parseFloat(heroProduct?.prices - heroProduct?.prices * heroProduct?.discount) || '000'}`}</p>
                                    </div>
                                    <div className={styles.offer_info}>
                                    <p className={styles.mrp_line}>{`₹${heroProduct?.prices || "000"}`}</p>
                                    <p className={styles.offer_line}>{`${heroProduct?.discount * 100}% off`}</p>
                                    </div>
                              </div>
                              <div className={styles.store_info}>
                                    <button onClick={() => navigate(`/home?store_id=${heroProduct.storeId}`)}>
                              ...
                              <div className={styles.description}>
                                    <p>Description</p>
                                    <span>
                                    {heroProduct?.description || "Description"}
                                    </span>
                              </div>
